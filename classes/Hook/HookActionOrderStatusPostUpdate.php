...
class HookActionOrderStatusPostUpdate implements HookInterface
{
    ...
    public function run()
    {
        ...
        $gaCancelledStates = json_decode(Configuration::get('GA_OLDCANCELLED_STATES'), true);
        ...
        $gaRefundSent = Db::getInstance()->getValue(
            'SELECT id_order FROM `' . _DB_PREFIX_ . 'oldganalytics` WHERE id_order = ' . (int) $this->params['id_order'] . ' AND refund_sent = 1'
        );

        // If it was not already refunded
        if ($gaRefundSent === false) {
            // We refund it and set the "sent" flag to true
            $this->context->cookie->__set('oldga_admin_refund', 'MBG.refundByOrderId(' . json_encode(['id' => $this->params['id_order']]) . ');');
            $this->context->cookie->write();

            // We save this information to database
            Db::getInstance()->execute(
                'UPDATE `' . _DB_PREFIX_ . 'oldganalytics` SET refund_sent = 1 WHERE id_order = ' . (int) $this->params['id_order']
            );
        }
    }
