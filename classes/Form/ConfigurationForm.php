...
namespace PrestaShop\Module\Ps_Oldgoogleanalytics\Form;
...
class ConfigurationForm
{
    ...
    public function __construct(Ps_Oldgoogleanalytics $module)
    {
        $this->module = $module;
    }
    ...
    public function generate()
    {
        ...
        $fields_form[0]['form'] = [
            ...
            'input' => [
                ...
                [
                    ...
                    'name' => 'GA_OLDUSERID_ENABLED',
                    'values' => [
                        [
                            'id' => 'GA_OLDUSERID_ENABLED',
                            'value' => 1,
                ...
                [
                    ...
                    'name' => 'GA_OLDTRACK_BACKOFFICE_ENABLED',
        ...
        if ($is_multistore_active) {
            $fields_form[0]['form']['input'][] = [
                ...
                'name' => 'GA_OLDCROSSDOMAIN_ENABLED',
                'values' => [
                    [
                        'id' => 'GA_OLDCROSSDOMAIN_ENABLED',
                        'value' => 1,
                        'label' => $this->module->l('Yes'),
                    ],
                    [
        ...
        // Load current value
        $helper->fields_value['GA_OLDACCOUNT_ID'] = Configuration::get('GA_OLDACCOUNT_ID');
        $helper->fields_value['GA_OLDUSERID_ENABLED'] = Configuration::get('GA_OLDUSERID_ENABLED');
        $helper->fields_value['GA_OLDCROSSDOMAIN_ENABLED'] = Configuration::get('GA_OLDCROSSDOMAIN_ENABLED');
        $helper->fields_value['GA_OLDANONYMIZE_ENABLED'] = Configuration::get('GA_OLDANONYMIZE_ENABLED');
        $helper->fields_value['GA_OLDTRACK_BACKOFFICE_ENABLED'] = Configuration::get('GA_OLDTRACK_BACKOFFICE_ENABLED');
        $helper->fields_value['GA_OLDCANCELLED_STATES[]'] = json_decode(Configuration::get('GA_OLDCANCELLED_STATES'), true);
    ...
    {
        $treatmentResult = '';
        $gaAccountId = Tools::getValue('GA_OLDACCOUNT_ID');
        $gaUserIdEnabled = Tools::getValue('GA_OLDUSERID_ENABLED');
        $gaCrossdomainEnabled = Tools::getValue('GA_OLDCROSSDOMAIN_ENABLED');
        $gaAnonymizeEnabled = Tools::getValue('GA_OLDANONYMIZE_ENABLED');
        $gaTrackBackOffice = Tools::getValue('GA_OLDTRACK_BACKOFFICE_ENABLED');
        $gaCancelledStates = Tools::getValue('GA_OLDCANCELLED_STATES');

        if (!empty($gaAccountId)) {
            Configuration::updateValue('GA_OLDACCOUNT_ID', $gaAccountId);
            Configuration::updateValue('OLDGANALYTICS_CONFIGURATION_OK', true);
            $treatmentResult .= $this->module->displayConfirmation($this->module->l('Account ID updated successfully'));
        }

        if (null !== $gaUserIdEnabled) {
            Configuration::updateValue('GA_OLDUSERID_ENABLED', (bool) $gaUserIdEnabled);
        ...
        if (null !== $gaCrossdomainEnabled) {
            Configuration::updateValue('GA_OLDCROSSDOMAIN_ENABLED', (bool) $gaCrossdomainEnabled);
            $treatmentResult .= $this->module->displayConfirmation($this->module->l('Settings for User ID updated successfully'));
        }

        if (null !== $gaAnonymizeEnabled) {
            Configuration::updateValue('GA_OLDANONYMIZE_ENABLED', (bool) $gaAnonymizeEnabled);
        ...
        if (null !== $gaTrackBackOffice) {
            Configuration::updateValue('GA_OLDTRACK_BACKOFFICE_ENABLED', (bool) $gaTrackBackOffice);
        ...
        if ($gaCancelledStates === false) {
            Configuration::updateValue('GA_OLDCANCELLED_STATES', '');
        } else {
            Configuration::updateValue('GA_OLDCANCELLED_STATES', json_encode($gaCancelledStates));
        }
